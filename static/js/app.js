...
async function loadPlaces() {
  ...
  places.forEach((place) => {
    ...
    card.innerHTML = `
      ...
      <button data-id="${place.id}" class="route-btn">Show Route</button>
    `;
...
async function drawRoute(place) {
  ...
  routeLine = L.polyline(route.geometry, { color: "#00a66b", weight: 5, opacity: 0.85 }).addTo(map);
  map.fitBounds(routeLine.getBounds(), { padding: [30, 30] });

  const alternatives = (route.alternatives || [])
    .map((a, idx) => `Alt ${idx + 1}: ${a.distance_km} km / ${a.duration_min} min`)
    .join("<br>");

  const trafficLevel = ["Low", "Medium", "High"][Math.floor(Math.random() * 3)];
  const wait = trafficLevel === "High" ? "8-12 min" : trafficLevel === "Medium" ? "3-7 min" : "0-2 min";
...
async function updateStats() {
  const response = await fetch(`/api/stats?lat=${userLocation[0]}&lng=${userLocation[1]}`);
  const stats = await response.json();
  document.getElementById("stats").textContent = `Total: ${stats.total} | Shortest: ${stats.shortest} km | Longest: ${stats.longest} km`;
}

function updateTrafficIndicator() {
  const traffic = ["Low", "Medium", "High"][Math.floor(Math.random() * 3)];
  const crowd = ["Low", "Medium", "High"][Math.floor(Math.random() * 3)];
  document.getElementById("traffic-indicator").textContent = `Traffic: ${traffic} | Crowd: ${crowd}`;
}
...
function setupLocationHandlers() {
  document.getElementById("detect-location-btn").onclick = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => alert("Could not detect location. Using default city center.")
    );
  };

  document.getElementById("apply-manual-location-btn").onclick = () => {
    const text = document.getElementById("manual-location").value.trim();
